...
Вариант 1 (рекомендуемый) — через DVC из публичного хранилища:

```bash
dvc pull
```

Данные будут загружены из Cloudflare R2 без необходимости учетных данных.

Вариант 2 — через DVC pipeline (загрузка с Kaggle):
